...
| 워터마크가 있으면 | 기대 수만큼 도착할 때까지 확정 보류(`awaiting_triggers`). 도착 후에도 유예(아래)는 별도로 적용된다 — 2026-09-03 ses-63에서 Node가 아직 인코딩 중인 트리거를 못 세어 적은 수치로 도착하자 유예 없이 즉시 확정되어 매출이 누락된 사고 이후 제거됨 |
| 기대 트리거가 끝내 안 오면 | `MODEL__CLOSE__BARRIER_TIMEOUT_S`(기본 10초)에서 에러 세션 — 조용한 매출 누락 대신 명시적 에러(fail-closed) |
| 유예 창 | **워터마크 유무와 무관하게 항상 적용**(`MODEL__CLOSE__GRACE_S`, 기본 5.0). `max(CLOSE 시각, 마지막 트리거 도착 시각) + 5초`까지 확정을 보류하고 `close_grace_pending`으로 응답 |
| 하위호환 | 선택 필드이므로 Node 무변경으로도 동작한다. 파싱 불가한 값은 무시된다(부가 신호 원칙) |

유예 창이 워터마크와 무관하게 항상 적용되는 이유: 워터마크(`expected_triggers`)는 Node가
아는 만큼만 정확하고, 아직 인코딩 중인 트리거는 Node의 녹화 디렉토리 카운트에도
잡히지 않을 수 있어(2026-09-03 ses-63) 이 정보만으로 유예를 생략하면 다시 매출이 누락될 수
있습니다.
