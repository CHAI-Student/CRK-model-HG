...
### 3.3 CRK-model (master) — 핵심 변경, 구조적으로 수용 가능 ✅
...
- 모든 보정은 `notes`에 사유 코드 기록 (I8):
  `zone2:cross_zone_vision_penalty:demoted=A:source=zone1@t0`.
...
1. **Phase 1 — 계측 (판정 영향 0)**
   CRK-CAMERA `change_timestamps` 패치 + 모델 optional 수용·저널(JSONL) 기록.
   실기에서 "타 존 오염 창 겹침"이 실제로 얼마나 발생하는지, 그때 vision
   candidates에 타 존 상품이 실제로 등장하는지 빈도를 정량화.
2. **Phase 2 — shadow 검증**
   `ShadowSettlerRunner`로 페널티 정산기를 shadow에 배치, diff만 수집.
   G2.5 저널 replay로 과거 세션 재생 검증 병행.
3. **Phase 3 — 전환**
   diff가 개선 방향임을 확인 후 primary 승격. α·ε·θ는 env 튜닝 노브로.
