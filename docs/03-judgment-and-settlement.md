...
| `PARTIAL_MIN_CONFIDENCE` | 0.18 | 무게 미검증 partial 청구의 conf 하한 (② near-gate·9.2·9.4). near-gate 미달은 후단 우회 없이 NO_DETECTION | 5표/conf 0.157 identity partial 및 이슈 #26 ses-25의 2표 동률/conf 0.0927 near-gate 오과금 | 0 |
...
| ⑤ | **확신 스냅 보호** (14차) | 게이트 **안** 스냅을 콤보가 뒤집으려면 존 판정(COMPLETE) conf < 0.92. 오버라이드 오답 6건은 전부 conf 0.96~1.0, 보호 케이스는 0.90/0.72 | `MODEL__CLOSE__COMBO_OVERRIDE_MAX_CONF` (>1로 비활성) |
...
| 무게 모호성 게이트 | `abs(delta)`를 게이트 내로 설명하는 (상품, 개수) 해가 **2종 이상**일 때만 페널티 발동. 무게가 유일 해를 지지하면 개입하지 않음 (무게 단서 > 비전 페널티). 이 KEEP은 **원 판정이 COMPLETE일 때만** — "무게 매칭이 이미 방어했다"는 전제가 무게 무검증 PARTIAL에는 거짓이므로 PARTIAL 원 판정은 게이트를 건너뛰고 재판정. 재판정도 COMPLETE가 아니고 실제 과금 상품이 오염 클래스이며, 페널티 후 동일 PARTIAL이 생존하지 못했을 때만 해당 상품 제거(이슈 #27 ses-43/44). 동일상품 PARTIAL 재승리·오염과 무관한 PARTIAL·COMPLETE는 보존 | — |
