...
- **Jetson 실기 E2E 검증 완료 (2026-07-09)**: 냉동 실기에서 OPEN → 트리거 추론(freezer_vision_first) → CLOSE 정산 → Node 결제 연동까지 통과 (이슈 #6 전 과정). 잔여 실기 항목: 24h+ soak(G4), side 카메라 ROI 튜닝(`SIDE_ROI_MAX_CENTER_X` — 현재 side 검출 대부분이 ROI에서 제거됨), 엣지 워터마크 실기 관찰(Edge_Environment `feature/edge-watermark` 브랜치)
