...
- **cold start (이슈 #9)**: 미지 셀은 그 트리거의 V∩W(비전 후보 × 무게 후보)로
  즉석 판별한다. V∩W가 복수면 **득표순 첫 게이트 통과 후보를 채택**한다 —
  v1 freezer_vision_first의 순차 시도 의미론. 유일성만 요구하면 신념이 빈 초기
  구간에 전 트리거가 pending으로 빠져 매출 누락 + 학습 교착이 된다(이슈 #9
  실기 확인). 트리거 채택(`vision_first_cold_start`)은 interim 표시·신념 증거
  용이고, **과금은 close에서 셀 net을 세션 비전 득표순으로 재해석**해 확정한다
  (`cell_net_vision_rank`). cold start 채택의 신념 가중치는 낮게(0.5) 쌓아
  잘못된 리더가 굳는 속도를 늦춘다. 1·2위 완전 동률(득표·conf 동일)만 판별
  불능으로 보류하며, 그 경우 청구하지 않는다 (fail-closed).
...
| `cell_net_vision_rank:zone{N}:ch{c}:{상품ID}={n}` | 미지 셀을 세션 비전 득표순 ∩ 게이트로 확정 (cold start 과금 경로, 이슈 #9) | 득표 1위가 실물과 맞는지 — 신념 학습 전 구간의 정상 경로 |
| `cell_net_vision_tie:zone{N}:ch{c}:{g}` | 득표 1·2위 완전 동률 — 순위 근거 없어 보류 | 수동 확인 (드묾 — 반복되면 카메라/모델 점검) |
