...
- 로컬 검증 게이트 (2026-07-08): `pytest tests -q` → **80 passed**
- G0(정적/단위) 수준 커버: 불변식 I1~I17 전건, E2E(OPEN→trigger→CLOSE→결제 페이로드), G2.5 훅(저널 replay 등가성)
- 미커버 (착수 전 확보물 P1~P5 대기): 게이트/조기종료 임계값 실측(P1 코퍼스), 세션 아카이브 replay(P2), interim 의미론·에러 정책 Node 합의(P3·P4), 카메라 seq 펌웨어(P5)
- 원본 대비 의도적 미구현: FastAPI HTTP 바인딩(파사드 `ModelService`를 감싸는 무로직 어댑터로 별도 작성), AVI 디코드/NVDEC(frames 입력은 디코드 완료 전제), YOLO TensorRT 로딩(Detector 어댑터)
...
cd CRK-model-HG
pytest tests -q        # 의존성: pytest만 (런타임 의존성 없음)
```
...
flowchart LR
    ...
    subgraph HG["CRK-model-HG"]
        direction TB
        ADPT["HTTP 어댑터 (무로직)<br/>FastAPI 등 — 이 레포 밖"]
