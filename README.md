...
| `MODEL__VISION__FREEZER_ROI_VERTICAL_REGION` | `upper` | dual-top 수직 ROI가 유지할 절반 (`upper`\|`lower`), 분할선은 `FREEZER_ROI_Y_SPLIT`(240, left-crop 좌표계) |
| `MODEL__VISION__TOP_ROI_ENABLED` | 0 | 냉장(dual) 레이아웃 top 카메라 하단 ROI — delta 있을 때 center_y ≥ 240만 유지 (원본 기본 true, HG는 냉동 실기 우선이라 off) |
| `MODEL__VISION__HAND_CONFIDENCE_THRESHOLD` | 0.30 | 손 검출 conf 하한 (P1-7) — 유령 손의 래치·hand_path 오염 차단. 0 = off |
| `MODEL__LOADCELL__ANALYZER` | `plateau` | primary 로드셀 분석기 (`plateau`\|`bocpd`) — 이슈 #14 승격 스위치. shadow 실측(analyze-sessions) 우세 확인 후에만 bocpd로 |
